...
public:
    ...
    T end() const { return window[(_end - 1 + size) % size]; }
...
// template<typename T, size_t size>
// std::ostream& operator<<(std::ostream& os, const WindowHistory<T, size>& wh) {
//     os << "[ ";
//     for(size_t i = 0; i < wh.win_size; i++) {
//         os << wh[i] << ' ';
//     }
//     return os << "] beg: " << wh.beg() << " end: " << wh.end() << " size: " << wh.curr_size();
// }